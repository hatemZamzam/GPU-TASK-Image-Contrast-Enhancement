...
int gpu_hist(int *h_hist, unsigned char * h_img_in, int nbr_bin, int img_size){
    ...
    cudaMemcpy(d_hist, h_hist, nbr_bin*sizeof(int), cudaMemcpyHostToDevice);

    int numThreads = 1024;
    int blockSize = (img_size/numThreads)/3; 

    gpu_histogram<<<blockSize, numThreads>>>(d_hist, d_img_in, nbr_bin, numThreads, img_size);

    cudaMemcpy(hist, d_hist, nbr_bin*sizeof(int), cudaMemcpyDeviceToHost);
...
PPM_IMG gpu_contrast_enhancement_c_yuv(PPM_IMG img_in)
{
    ...
    hist = gpu_hist(hist, yuv_med.img_y, 256, yuv_med.h*yuv_med.w);
...
PPM_IMG gpu_contrast_enhancement_c_hsl(PPM_IMG img_in)
{
    ...
    hist = gpu_hist(hist, hsl_med.l, 256, hsl_med.height*hsl_med.width);
